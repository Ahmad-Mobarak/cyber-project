...
# Try to import utils with different methods
try:
    # First try absolute import (for PyInstaller)
    from Modules.utils import err_exit, user_confirm, chk_wlist
except ImportError:
    try:
        # Then try relative import (for regular Python)
        from utils import err_exit, user_confirm, chk_wlist
    except ImportError:
        # If both fail, add the current directory to the path and try again
        sys.path.append(os.path.dirname(os.path.abspath(__file__)))
        try:
            from utils import err_exit, user_confirm, chk_wlist
        except ImportError:
            print("Fatal error: Cannot import utils module")
            sys.exit(1)
    ...
    def Structure(self):
        ...
        try:
            if not zipfile.is_zipfile(self.targetFile):
                print(f"{errorS} Not a valid ZIP-based document file")
                return
            ...
            try:
                ...
                print(f"{errorS} Error parsing document structure: {str(e)}")
                return

            # Perform analysis against binaries
            if bins:
                print(f"\n{infoS} Analyzing embedded binary files...")
                for b in bins:
                    try:
                        bdata = document.read(b)
                        self.BinaryAnalysis(b, bdata)
                    except Exception as e:
                        print(f"{errorS} Error analyzing binary {b}: {str(e)}")

            # Check for interesting external links
            ...
            for fff in document.namelist():
                try:
                    ...
                    print(f"{errorS} Error extracting links from {fff}: {str(e)}")
                    continue
            
            if link_count > 0:
                print(exlinks)
            else:
                print(f"{infoS} No interesting links found.")
            
        except zipfile.BadZipFile:
            print(f"{errorS} Error: Invalid or corrupted ZIP file.")
        except Exception as e:
            print(f"{errorS} Error analyzing document structure: {str(e)}")
    ...
    def MacroHunter(self):
        ...
        try:
            ...
            if macro_state_vba != 0 or macro_state_xlm != 0:
                # We are removing the user_confirm prompt to make it GUI-compatible
                print(f"\n{infoS} Macros found. Automatically extracting...")
